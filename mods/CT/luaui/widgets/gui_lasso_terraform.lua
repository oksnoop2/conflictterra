		...
		if button == 1 then
			if not spIsAboveMiniMap(mx, my) then
				...
				if pos then	
					...
					drawingRamp = 1
					terraform_type = 4
					terraformHeight = minRampWidth -- width
					mouseX = mx
					mouseY = my
					return true
...
local function drawMouseText(text)

	local mx,my = Spring.GetMouseState()
	glText(text, mx+50, my, 22,"cn")
...
function widget:DrawScreen()
	...
	if terraform_type == 1 or terraform_type == 2 then
		if setHeight then
			drawMouseText(floor(terraformHeight))
		end
	elseif terraform_type == 4 then
		if drawingRamp == 1 then
			drawMouseText(floor(point[1].y))
		elseif drawingRamp == 3 then
			if point[2].y == 0 then
				drawMouseText(point[2].y .. " Water Level")
			elseif point[2].y == point[1].y then
				drawMouseText(floor(point[2].y) .. " Flat")
			else
				drawMouseText(floor(point[2].y))
			end
		end
	end
