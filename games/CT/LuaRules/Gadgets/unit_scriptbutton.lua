...
function gadget:GetInfo()
  return {
    name      = "ScriptButtons",
    desc      = "Easy script button creation",
    author    = "quantum (CarRepairer - Upgraded to LUS)",
    date      = "June 28, 2008",
...
local buttonDefs = VFS.Include "LuaRules/Configs/script_buttons.lua"
...
function gadget:GameFrame(n)
	...
	for unitID, unitButtons in pairs(reloads) do
		...
		if (not status or not udfid) then
			reloads[unitID] = nil
		else
			...
			for buttonIndex, status in pairs(unitButtons) do
				...
				if (cmdDescID) then
					...
					if (cmd.duration and 
						...
						if (cmd.endscript and not expired) then
							--Spring.CallCOBScript(unitID, cmd.endscript, 0)
							CallUnitScript(unitID, cmd.endscript)
							status[2] = true
						end
	...
	if (buttons[cmdID] and buttonDefs[UnitDefs[unitDefID].name]) then
		...
		if (cmd.reload) then
			...
			if (not reloads[unitID][cmd.buttonIndex]) then
				...
				CallUnitScript(unitID, cmd.startscript)
			end
		else
			--Spring.CallCOBScript(unitID, cmd.cob, 0)
			CallUnitScript(unitID, cmd.startscript)
