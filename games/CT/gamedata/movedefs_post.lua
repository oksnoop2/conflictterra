...
local tagIgnores =	tags.tagIgnoreList
	...
	if (ud.movementclass) then
		...
		if (myMovDef ~= nil ) then -- found a valid def time to error check that.
			...
			tagName = string.lower(tagName)
				if ( MoveDefs[myMovDef][tagName] ~= nil and tagIgnores[tagName] == nil ) then
					if ( ud[tagName] ~= nil and tonumber(ud[tagName]) ~= tonumber(MoveDefs[myMovDef][tagName]) ) then
